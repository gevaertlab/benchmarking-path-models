...
def frozen_features(arch, model, images, n, avgpool, batch_size):
    with torch.no_grad():
        with torch.cuda.amp.autocast(dtype=torch.float16):
            if "vit" in arch or arch == "H0-mini":
                individual_patch = images.view(-1, images.size(2), images.size(3), images.size(4))
                print('individual_patch shape', individual_patch.shape)
                output = model(individual_patch)  # [batch_size * bag_size, num_tokens, embed_dim]
                print('output shape is ..', output.shape)
                if arch == "H0-mini":
                    cls_features = output[:, 0]  # [batch_size * bag_size, 768]
                    patch_features = output[:, model.num_prefix_tokens:]  # [batch_size * bag_size, num_patches, 768]
                    reshaped_patch = cls_features.reshape(batch_size, -1, cls_features.size(-1))  # [batch_size, bag_size, 768]
                    reshaped_patch_mean = patch_features.reshape(batch_size, -1, patch_features.size(-1)).mean(dim=1)  # [batch_size, 768]
                    output = torch.cat([reshaped_patch.mean(dim=1), reshaped_patch_mean], dim=-1)  # [batch_size, 1536]
                else:
    ...
    for i, (data_train_fold, data_val_fold) in enumerate(zip(dataset_train_list, dataset_val_list)):
        ...
        for epoch in range(start_epoch, args.epochs):
            ...
            if epoch % args.val_freq == 0 or epoch == args.epochs - 1:
                ...
                print(f"precision at epoch {epoch} of the {len(dataset_val)} val images for {fold}: {test_stats['precision']:.1f}%")
                print(f"recall at epoch {epoch} of the {len(dataset_val)} val images for {fold} : {test_stats['recall']:.1f}%")
                print(f"balanced_accuracy at epoch {epoch} of the {len(dataset_val)} val images for {fold} : {test_stats['balanced_accuracy']:.1f}%")
...
def validate_network(has_frozen, arch, val_loader, model, linear_classifier, n, avgpool, batch_size, outputdir):
    ...
    output_preds['outputs'] = np.concatenate(output_preds['outputs'], axis=0)
