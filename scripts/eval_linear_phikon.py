...
def frozen_features(arch,model,images,n , avgpool,batch_size):
    with torch.no_grad():
        if "vit" in args.arch:
            ...
            intermediate_output = model.get_intermediate_layers(individual_patch, n)
            print('intermediate output shape iss..' , len(intermediate_output))
...
def validate_network(has_frozen, arch, val_loader, model, linear_classifier, n, avgpool,batch_size,outputdir):
    ...
    output_preds['preds'] = np.array(output_preds['preds']).flatten()
    print('len of output preds is ..', len(output_preds['preds']))
    print('output preds is ..', output_preds['preds'])
    output_preds['targets'] = np.array(output_preds['targets']).flatten()
    output_preds['outputs'] = np.array(output_preds['outputs']).flatten() 
